...
class Form {
	 ...
	 * Checkboxes
	 *
	 * Generates a list of label grouped checkboxes (great for JS enhancement)
	 *
	 * Form::checkboxes('tags', $checked_array, $tag_options);
	 *
	 * @param	string|array	either fieldname or full attributes array (when array other params are ignored)
	 * @param	string|array	checked, string value or array of values
	 * @param	array			options
	 * @param	array			attributes
	 * @return	string
	 */
	public static function checkboxes($field, $values = null, Array $options = array(), $label, Array $attributes = array())
	{
		if (is_array($field))
		{
			$attributes = $field;
		...
		{
			$attributes['name'] = (string) $field;
			$attributes['checked'] = $values; //can be an array
			$attributes['options'] = $options;
		}
		...
		if ( ! isset($attributes['options']) || ! is_array($attributes['options']))
		{
			throw new \Fuel_Exception(sprintf('Checkbox list "%s" is either missing the "options" or "options" is not array.', $attributes['name']));
		}

		if ( count($attributes['options']) > 1 && !preg_match("/\[\]$/",$attributes['name']) )
		{
			$attributes['name'] .= "[]";
		}
		...
		// Get the checked items then unset it from the array
		$checked = array_values((array) (empty($attributes['checked']) ? '' : $attributes['checked']));
		unset($attributes['checked']);

		$input = PHP_EOL;
		$input .= "<fieldset><legend>".$label."</legend>";
		foreach ($options as $key => $val)
		{
			$opt_attr = array('value' => $key);
			(in_array($key,$checked)) && $opt_attr[] = 'checked';
			$input .= str_repeat("\t", 1);
			$opt_attr['value'] = (static::get_class_config('prep_value', true) && empty($attributes['dont_prep'])) ?
				static::prep_value($opt_attr['value']) : $opt_attr['value'];

			$input .= "<label>".static::checkbox($attributes['name'], $key, array_merge($attributes,$opt_attr))." ".$val."</label>".PHP_EOL;
		}
	...
	public function build_field($field)
	{
		...
		switch($field->type)
		{
			...
			case 'checkboxes':
				$attributes = $field->attributes;
				unset($attributes['type']);
				return static::checkboxes($field->name,$field->value ,$field->options, $field->label, $attributes);
			default:
