...
class Model_Property_Validation
{
	...
	{
		return Fieldset::factory('add_property')
				->add_model('Model_Property_Validation', null, 'set_add_form')
				->repopulate();
	}
	...
	{
		return Fieldset::factory('edit_property')
				->add_model('Model_Property_Validation', $property, 'set_edit_form')
				->repopulate();
	}
	...
	public function _validation_unique($value, $field, $updated_id = false)
    {
		if($updated_id)
		{
			$same = Model_Property::find()->where($field, '=', $value)->get_one();
			//FIXME test this method call
			return ($same->id == $updated_id);
		}
		else
		{
			$count = Model_Property::find()->where($field, '=', $value)->count();
			return ($count == 0);
		}
    }
